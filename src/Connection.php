...
class Connection extends IlluminateConnection {
	...
	public function createConnection()
	{
		$client = ClientBuilder::create()
			->addConnection($this->getConnectionType(), $this->getConnectionUrl())
			->build();
	...
	{
		return $this->getConfig('connection') ?? 'default';
	}

	/**
	 * Get GraphAware client url
	 *
	 * @return string
	 */
	public function getConnectionUrl()
	{
		return $this->getProtocol() . $this->getUsername() . ':' . $this->getPassword() . '@' . $this->getHost() . ':' . $this->getPort();
	}
	...
	{
		if ($this->getConnectionType() == 'bolt')
			return 'bolt://';
	...
	{
		return $this->client->run($query, $this->prepareBindings($bindings));
	}
	 ...
	 *
	 * @return \Ahsan\Neoquent\Query\Grammars\CypherGrammar
	 */
